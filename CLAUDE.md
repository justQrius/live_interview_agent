...
**Status**: Phase 1-3 Complete, Phase 4-6 Implemented
...
│       │   ├── AccumulatingIndicator.tsx # Phase 6: Buffering state
│       │   ├── AnswerDisplay.tsx
...
│       ├── classification/ # Question detection, reformulation (TopicStack), splitting, utterance accumulation
...
### Phase 6 Features (Utterance Accumulation)
- **Multi-Segment Question Detection**: Accumulates speech across VAD-induced pauses
- **4-Tier Completeness Detection**: Punctuation → Syntax → Timing → LLM semantic check
- **Per-Speaker Buffering**: Separate buffers with configurable soft/hard timeouts
- **Real-time Accumulation UI**: AccumulatingIndicator shows buffering state
- **Environment Config**: All thresholds configurable via env vars

### Phase 7 Features (Gen 1 2026 Streaming)
- **Real-time Streaming STT**: WebSocket-based instant transcription (~150ms latency)
- **Semantic Endpointing**: Intelligent turn detection using meaning, not just silence
- **Hybrid Mode**: Combines streaming semantic endpointing with timing-based accumulation
- **2026 Gen 1 Models**: Support for Nova-3, GPT-5.2, Claude 4 Opus, Gemini 3 Pro
- **High Reasoning Mode**: Extended thinking toggle for complex logic tasks
- **Streaming Providers**: Deepgram (Nova-3), AssemblyAI (V3), OpenAI Realtime (GA)
